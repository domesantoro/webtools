#!/usr/bin/env bash
# Controllo del server webtools_workspaces: --start | --stop
#
# - --start: avvia in background (nohup), slegato dal terminale.
#            PID in webtools_workspaces.pid, log in webtools_workspaces.log (in append).
...
PID_FILE="$DIR/webtools_workspaces.pid"
LOG_FILE="$DIR/webtools_workspaces.log"
...
start() {
  ...
  if pid="$(running_pid)"; then
    echo "webtools_workspaces è già in esecuzione (PID $pid)."
    return 0
  fi
  if [[ -z "$NODE" ]]; then
  ...
  for _ in $(seq 1 50); do
    ...
    if tail -c +"$((log_offset + 1))" "$LOG_FILE" | grep -q "webtools_workspaces in ascolto su"; then
      echo "webtools_workspaces avviato (PID $pid). Log: $LOG_FILE"
      return 0
    fi
    sleep 0.2
  done
...
stop() {
  ...
  if ! pid="$(running_pid)"; then
    rm -f "$PID_FILE"
    echo "webtools_workspaces non è in esecuzione."
    return 0
  fi
  kill -TERM "$pid"
  for _ in $(seq 1 50); do
    if ! kill -0 "$pid" 2>/dev/null; then
      rm -f "$PID_FILE"
      echo "webtools_workspaces fermato (PID $pid)."
      return 0
    fi
    sleep 0.2
  done
