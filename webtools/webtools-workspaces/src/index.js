// Avvio del server.
//
// In background: ./webtools_workspaces.sh --start
...
try {
  ...
  console.error(`webtools_workspaces non parte: ${error.message}`);
  process.exit(1);
}
...
server.listen(settings.port, settings.host, () => {
  ...
  console.log(
    `webtools_workspaces in ascolto su http://${settings.host}:${settings.port} ` +
      `(radice: ${settings.root})`
  );
});
