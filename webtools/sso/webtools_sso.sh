#!/usr/bin/env bash
# Control of the webtools_sso server: --start | --stop
#
# - --start: starts it in the background (nohup), detached from the terminal.
#            PID in webtools_sso.pid, log in webtools_sso.log (appended).
...
start() {
  ...
  if pid="$(running_pid)"; then
    echo "webtools_sso is already running (PID $pid)."
    return 0
  fi
  if [[ -z "$NODE" ]]; then
  ...
  for _ in $(seq 1 50); do
    ...
    if tail -c +"$((log_offset + 1))" "$LOG_FILE" | grep -q "webtools_sso listening on"; then
      echo "webtools_sso started (PID $pid). Log: $LOG_FILE"
      return 0
    fi
    sleep 0.2
  done
...
stop() {
  ...
  if ! pid="$(running_pid)"; then
    rm -f "$PID_FILE"
    echo "webtools_sso is not running."
    return 0
  fi
  kill -TERM "$pid"
  for _ in $(seq 1 50); do
    if ! kill -0 "$pid" 2>/dev/null; then
      rm -f "$PID_FILE"
      echo "webtools_sso stopped (PID $pid)."
      return 0
    fi
    sleep 0.2
  done
