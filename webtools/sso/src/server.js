...
import { exchangeTicket, issueTicket, login, logout, readSession } from "./auth.js";
...
// Un corpo di login sano sta in poche centinaia di byte: oltre, si chiude.
...
async function returnWithTicket(response, settings, client, token, next, cookieHeader) {
  const service = serviceOf(next);
  const issued = await issueTicket(settings, token, service, client);
  if (!issued.ok) {
    const html = renderLoginPage({ next, error: "unavailable" });
...
async function showLoginPage(request, response, settings, client, url) {
  ...
  if (token) {
    ...
    if (stato.ok && stato.body.logged) {
      return returnWithTicket(response, settings, client, token, next, null);
    }
  }

  return sendHtml(response, 200, renderLoginPage({ next }));
}
...
async function handleLoginForm(request, response, settings, client, url) {
  ...
  if (!username || !password) {
    return sendHtml(response, 400, renderLoginPage({ next, username, error: "invalid_credentials" }));
  }

  const entrato = await login(settings, { username, password }, client);
  if (!entrato.ok) {
    const error = entrato.status === 401 ? "invalid_credentials" : "unavailable";
    // Lo username resta scritto, la password no: si ridigita.
    return sendHtml(response, entrato.status, renderLoginPage({ next, username, error }));
  }

  const token = entrato.body.session.token;
  const cookie = setCookie(settings.cookieName, token, settings.sessionTtlSeconds);
  return returnWithTicket(response, settings, client, token, next, cookie);
}
...
function showRegisterPage(request, response, settings, client, url) {
  const next = safeNext(settings, url.searchParams.get("next"));
  return sendHtml(response, 200, renderRegisterPage({ next }));
}
